...
def create_overview_charts(invoices_df, credit_notes_df, metrics):
    ...
    # Monthly Invoice Trends - Compact line chart
    invoices_df['month_year'] = invoices_df['invoice_date'].dt.to_period('M').astype(str)
    monthly_data = invoices_df.groupby('month_year').agg({
        'total': 'sum',
    ...
    fig_trends.update_layout(
        xaxis=dict(title="", tickangle=45, tickfont=dict(size=10), showgrid=False),
...
def create_credit_note_analysis(credit_notes_df):
    ...
    # Monthly Credit Note Trends - Compact line chart
    credit_notes_df['credit_month'] = credit_notes_df['Date'].dt.to_period('M').astype(str)
    monthly_credits = credit_notes_df.groupby('credit_month').agg({
        'Total': 'sum',
        'CreditNoteNumber': 'count'
    }).reset_index()
    
    fig_monthly_credits = go.Figure()
    fig_monthly_credits.add_trace(go.Scatter(
        x=monthly_credits['credit_month'],
        y=monthly_credits['Total'],
        mode='lines+markers',
        name='Credit Amount',
    ...
    fig_monthly_credits.update_layout(
        xaxis=dict(title="", tickangle=45, tickfont=dict(size=10), showgrid=False),
        yaxis=dict(title="Amount ($)", tickfont=dict(size=10), tickformat='$,.0f', showgrid=False),
        hovermode='x unified',
        showlegend=False,
    ...
    return fig_credit_status, fig_monthly_credits
